...
export default function ServiceCard({
  ...
  return (
    <Link href={href} className="card hover:shadow-md transition-shadow group flex flex-col">
      {/* Image or Icon */}
      {image ? (
      ...
      {features && features.length > 0 && (
        <ul className="space-y-2 mb-6">
          {features.map((feature, index) => (
            <li key={index} className="flex items-center gap-2 text-[#4B4B4B] text-sm">
      ...
      {/* Link */}
      <span className="inline-flex items-center gap-1 text-[#C9653B] font-semibold text-sm mt-auto">
        Learn more
